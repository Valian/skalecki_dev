...
I'm extremely happy to announce that LiveVue hit 1.0 today! I've built a dedicated page showing all the major capabilities of the library that can be found here: [livevue.skalecki.dev](https://livevue.skalecki.dev)

![LiveVue 1.0](/images/live_vue_screenshot.png)

LiveVue is a library that allows you to seamlessly mix Phoenix LiveView and Vue.js. This post is my attempt to explain reasons for this library to exist, decisions made along the way and technical challenges I've had to overcome to reach 1.0. First public commit happened on May 8, 2024. Looking back, it's quite a long time ago.

## The Problem

It was 2024, Phoenix 1.7. I'd freshly left the Python world for Elixir. I was building an app with Phoenix LiveView and kept running into the same challenge.

My app had dynamic forms - conditional sections visible based on other values and some client-side interactions that didn't need to go to the server. Creating these with LiveView and Live Components was quite hard. I wanted lightweight per-component state — similar to what I was used to in the "old" days, using Vue or React. Creating hooks was time-consuming, JS commands felt brittle, Live Components felt heavy for what I needed. And many round-trips my app was making felt unnecessary.

In other words, I couldn't find a good option for declarative client-side rendering in Phoenix.

So I went looking for a better solution. First, I stumbled upon Alpine.js. While easy to integrate, I still had a feeling it's not exactly what I'm looking for. Then, I found a wonderful library [LiveSvelte](https://github.com/woutdp/live_svelte) and immediately saw the potential. But I'm a longtime Vue fan — I wanted that familiar development experience in Phoenix.

If LiveSvelte could exist, why not LiveVue?
...
Something that worked for Svelte, didn't work that well for Vue - I don't know the reason to that day. The esbuild setup required two separate processes — one for the SSR bundle, one for the client bundle. Each change triggered both builds, then the browser needed a full page reload. Even for a very simple project, compilation took 1-2s, and of course there was no hot reload - something I valued highly when building web apps.

I knew Vite could do better. It can serve both SSR and client bundles from memory with instant hot module replacement. There were two changes needed:

1. Serve static assets from Vite dev server instead of priv/static directory
2. Expose additional SSR endpoint, consumed by Phoenix in development. I had to write a [custom vite plugin](https://github.com/Valian/live_vue/blob/main/assets/vitePlugin.js#L98) so it would expose additional SSR endpoint.

Last piece of the puzzle was to [enable hot reload of Phoenix LiveView](https://x.com/jskalc/status/1788308446007132509). This requires some tweaks to Phoenix config, but the end result is stateful hot reload across the whole stack 🤯

## How LiveVue works

To use LiveVue, you need two things:

1. A regular Vue component, located either in `assets/vue` directory or somewhere in `lib/my_app_web`. Example of a simple component:
...
2. Then you can render it in your LiveView using `<.vue>` component:
...
What actually happens? The `.vue` component generates a div with data attributes:
...
The magic is in `updated()`. When server state changes, LiveView sends new data over WebSocket. Phoenix updates the `data-props` attribute. The hook reads the new props and assigns them to the reactive object. Vue detects the changes and re-renders only what's needed.

Server state flows down as props, client events flow up to the server. That's the core loop.
...
The basic architecture worked. People started using it! That was a great feeling. But I wasn't satisfied. Every time props changed, LiveView was sending **the entire props object**. For a component with a list of 100 items, changing one item meant re-sending all 100. That's wasteful.

### Step 1: Sending only changed attributes

LiveView has change tracking. The `assigns.__changed__` map tells you which assigns were modified. I used this to manually construct `__changed__` map for each data-attribute, so they were not sent if nothing changed.
...
After that change, LiveView was sending only changed attributes. It was a nice improvement, but not yet there - changing even a single prop meant sending the entire props object. I wanted to diff it as well!
...
I [submitted a PR to Phoenix LiveView](https://github.com/phoenixframework/phoenix_live_view/pull/3392) to store previous values for complex assigns. It was accepted. Now I had before-and-after state!
...
However, there was a problem — previously I was simply dumping props into JSON. With diffs, I can't do it - it's impossible to generate a patch from a JSON string, I would need to decode it first. Instead I've opted to implement a custom `LiveVue.Encoder` protocol to turn structs into plain maps and lists.

Then, I could construct diffs. It works by comparing the previous and new value and generating a list of operations to transform the previous value into the new one, such as:
...
As a last step, I've implemented a custom `updated` hook that applied that diff to the props. In some cases, payload sizes dropped by 90% or more ❤️ In the meantime I've implemented [a number of PRs](https://github.com/corka149/jsonpatch/issues?q=state%3Aclosed%20is%3Apr%20author%3A%40me) to jsonpatch library to support my case efficiently - encoding is now lazy, only if a given value was changed.
...
Seven operations to insert one item... The algorithm compared items by index. Since everything shifted, everything "changed". Far from ideal when I want to make it the default way of updating props.
...
One operation. Much better! The more complex data structure, the bigger the difference, since diff is performed recursively for matched items.

## Filling the Gaps

With the solid architecture and performance optimized, I wanted to cover the remaining LiveView features.

### Streams

LiveView streams have their own insertion/deletion semantics. I added custom diff operations and built a translation layer. On the Vue side, you just receive a reactive array:
...
Forms are why I built LiveVue in the first place. I wanted Ecto changeset validation to flow to the client without duplicating logic, but still be able to use client-side conditional rendering, animations and other goodies.

Designing forms API is far from trivial. In the past I've used [VeeValidate](https://vee-validate.logaretm.com/) and liked it. So I built `useLiveForm` with a similar idea:
...
The LiveView side is exactly the same as you would do with regular Phoenix form:
...
Behind the scenes, LiveVue serializes form errors and values into JSON. Relations and embedded schemas are supported. Composable also tracks additional state for each field, e.g., initialValue, isTouched, etc. Each time when user interacts with the form, props are updated and each field instance is automatically synchronized with updated data.

I think `useLiveForm` is a good abstraction because it doesn't enforce any specific UI patterns. It's up to the developer how to render the form, errors, what the logic to allow submission is, etc. LiveVue only gives you the data and the API to work with it, and handles all the complexity of asynchronous validation. See it in action [here](https://livevue.skalecki.dev/examples/simple-form?tab=preview).

What's even better, it's fully typed with TypeScript! For example, if you type `form.field('skills[0].name')` your IDE is able to verify if field path exists and warn you if you make a typo. The [implementation](https://github.com/Valian/live_vue/blob/main/assets/useLiveForm.ts#L60) is the craziest TypeScript I've ever written, but from the outside it's fairly simple.
...
That approach makes colocating Vue with LiveViews a natural thing to do.
...
I found [PhoenixVite](https://github.com/lostkobrakai/phoenix_vite) by Benjamin Milde (the beloved LostKobrakai) and built igniter installer on top of it:
...
I also built a VS Code extension for `~VUE` sigil syntax highlighting. I tried to add autocomplete and TypeScript support inside sigils, but it's really hard. If you want a challenge, [contributions welcome](https://github.com/Valian/live_vue)!

## The 1.0 Moment

What made me ready to call it 1.0?

I'm using LiveVue in production for over a year now, but always had a feeling "I'm missing something". Recently, it's... gone. Building apps with LiveVue became a joy. All the pieces are there — the Vue ecosystem, client-side state when needed, stateful server-side state from LiveView. Both can render dynamic HTML. I *could feel* it was complete.
...
During this journey, I was constantly delighted by the community response. Every update I shared on X or Elixir Forum was warmly welcomed. In addition, a few months ago I was able to present my perspective on **Why mixing LiveView and a frontend framework is a great idea** at [ElixirConf EU 2025](https://www.youtube.com/watch?v=oouX5lxf48k) and meet people like José Valim, Chris McCord, and many others.
...
I love being part of the Elixir community and wanted to give something back. Since I'm good at both Vue and Elixir, this is what I could contribute and I'm happy I've done it. Hope you'll find it useful!
